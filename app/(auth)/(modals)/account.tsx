...
const Page = () => {
  const user = { firstName: 'Ephraim', lastName: 'Imhagbe' };
  const [firstName, setFirstName] = useState(user?.firstName);
  const [lastName, setLastName] = useState(user?.lastName);
  const [edit, setEdit] = useState(false);

  const ImageSource =
    'https://th.bing.com/th/id/OIP._C8lpLQFe5OkZzguxJhYVAHaHa?rs=1&pid=ImgDetMain';

  const handleSave = () => {
    setEdit(false);
    // Logic to save updated data if needed.
  };
